...
const LessonCard = ({ title, activeLesson, completed, locked }) => {
    return (
        <div className={`flex items-center gap-3 p-4 cursor-pointer ${activeLesson ? "border-l-2 border-l-primary bg-primary-fade" : ""}`}>
            {completed ?
                <CircleCheckBig className={`w-4 font-medium ${activeLesson ? "text-primary" : completed ? "text-green-500" : ""}`} />
                :
                locked ?
                    <Lock className={`w-4 font-medium ${activeLesson ? "text-primary" : ""}`} /> :
                    <CirclePlay className={`w-4 font-medium ${activeLesson ? "text-primary" : ""}`} />
            }
